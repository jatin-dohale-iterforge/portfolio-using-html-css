...
const showLoaderStateCity = () => {
  document.querySelector("#state-cities-holder").classList.toggle("hidden");
  ...
  try {
    ...
    await data.find((item) => {
      if (item.name == country) {
        if (state == null) return (result = item.states[0]);

        item.states.find((s) => {
          if (s.name.replaceAll(" ", "") == state.replaceAll(" ", "")) {
            result = s;
            return true;
          }
        });
...
// function to get data of each city
const getEachCityData = async (country, state) => {
  showLoaderStateCity();
  console.log(await getCities(country, state));
  const cities = await getCities(country, state);

  try {
    let cityPromises = cities.top_cities.map((city) =>
      fetch(
        `https://geocoding-api.open-meteo.com/v1/search?name=${city.name}&count=1`,
      ).then((res) => res.json()),
    );
    const city = await Promise.all(cityPromises);

    console.log(city);

    let cityDataPromises = city.map((c) =>
      fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${c.results[0].latitude}&longitude=${c.results[0].longitude}&current_weather=true&timezone=auto`,
      ).then((res) => res.json()),
    );
    ...
    let minShowLength = Math.min(4, cityData.length);

    for (let index = 0; index < minShowLength; index++) {
      stateBox.innerHTML += `<div >
                <div>
                  <p>${city[index].results[0].name}</p>
                  <span>${Math.ceil(cityData[index].current_weather.temperature) + "°C"}</span>
                </div>
                <div>
                <span>
                 <img src=${getWeatherCode(cityData[index].current_weather.weathercode)} alt="" />
                </span>
                </div>
              </div>`;
    }
...
const getData = async (city = "mumbai") => {
  ...
  if (sessionStorage.getItem(city)) {
    ...
    try {
      ...
      getEachCityData(data.results[0].country, data.results[0].admin1);
...
const showData = async (cityWeatherData) => {
  ...
  const weatherTomorrow = getWeatherCode(
    cityWeatherData[1].daily.weathercode[1],
  )
    .split("/")[3]
    .split(".")[0]
    .replace("_", " ");

  tomorrowBox[0].children[1].innerText =
    weatherTomorrow.charAt(0).toUpperCase() + weatherTomorrow.slice(1);
  tomorrowBox[1].children[0].innerText =
    cityWeatherData[1].daily.temperature_2m_max[1] + "°C";
  tomorrowBox[2].children[0].children[0].src = getWeatherCode(
    cityWeatherData[1].daily.weathercode[1],
  );

  overlayBox[0].children[1].innerText =
    weather.charAt(0).toUpperCase() + weather.slice(1);
  overlayBox[1].children[0].innerText =
    cityWeatherData[1].daily.temperature_2m_max[0] + "°C";
  overlayBox[2].children[0].children[0].src = getWeatherCode(
    cityWeatherData[1].daily.weathercode[1],
  );
...
const stateBox = document.getElementById("state-cities-holder");
